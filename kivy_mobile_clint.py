...
class BlowfishCipher:
    ...
    def _f(self, x):
        h = _u32(self.s[0][(x >> 24) & 0xFF] + self.s[1][(x >> 16) & 0xFF])
        return _u32((h ^ self.s[2][(x >> 8) & 0xFF]) + self.s[3][x & 0xFF])

    def _enc_block(self, xL, xR):
        for i in range(16):
            xL = _u32(xL ^ self.p[i])
            xR = _u32(self._f(xL) ^ xR)
            xL, xR = xR, xL
        xL, xR = xR, xL
        xR = _u32(xR ^ self.p[16])
        xL = _u32(xL ^ self.p[17])
        return xL, xR

    def _dec_block(self, xL, xR):
    ...
    def _key_schedule(self, key: bytes):
        ...
        for i in range(18):
            data = 0
            for _ in range(4):
                data = _u32((data << 8) | key[j % len(key)])
                j += 1
            self.p[i] = _u32(self.p[i] ^ data)
        xL = xR = 0
        for i in range(0, 18, 2):
            xL, xR = self._enc_block(xL, xR)
            self.p[i], self.p[i + 1] = xL, xR
        for s in range(4):
            for i in range(0, 256, 2):
                xL, xR = self._enc_block(xL, xR)
                self.s[s][i], self.s[s][i + 1] = xL, xR
    ...
    def encrypt_cbc(self, plaintext: bytes, iv: bytes) -> bytes:
        ...
        for i in range(0, len(padded), 8):
            ...
            xL, xR = unpack(">II", block)
            prev = pack(">II", *self._enc_block(xL, xR))
            out += prev
        return out
    ...
    def decrypt_cbc(self, ciphertext: bytes, iv: bytes) -> bytes:
        ...
        for i in range(0, len(ciphertext), 8):
            ...
            xL, xR = unpack(">II", block)
            dec = pack(">II", *self._dec_block(xL, xR))
            out += bytes(a ^ b for a, b in zip(dec, prev))
            prev = block
        return self._unpad(out)

    def encrypt_message_cbc(self, msg: str, iv: bytes) -> bytes:
        return self.encrypt_cbc(msg.encode("utf-8"), iv)

    def decrypt_message_cbc(self, ct: bytes, iv: bytes) -> str:
...
def build_secure_packet(cipher, mac_key, plaintext, seq):
    iv = os.urandom(8)
    ct = cipher.encrypt_message_cbc(plaintext, iv)
    aad = seq.to_bytes(8, "big")
    tag = _hmac.new(mac_key, aad + iv + ct, hashlib.sha256).digest()
    return {
...
def parse_secure_packet(cipher, mac_key, packet, last_seq):
    if packet.get("version") != PROTOCOL_VERSION:
        raise ValueError("Bad protocol version.")
    seq = packet["seq"]
    if seq <= last_seq:
        raise ValueError("Replay packet rejected.")
    iv  = base64.b64decode(packet["iv"])
    ct  = base64.b64decode(packet["ciphertext"])
    tag = base64.b64decode(packet["hmac"])
    expected = _hmac.new(mac_key, seq.to_bytes(8, "big") + iv + ct, hashlib.sha256).digest()
    if not _hmac.compare_digest(tag, expected):
        raise ValueError("Integrity check failed — possible tampering!")
